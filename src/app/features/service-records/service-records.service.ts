...
import { ServiceRecord, ResultResult } from '../../shared/models/service-record.model';
...
export class ServiceRecordsService {
  ...
  readonly record = signal<ResultResult | null>(null);
  ...
  async fetchServiceRecord(gamertag: string): Promise<void> {
    ...
    if (data) {
      this.record.set(data.Results[0].Result);
    }
  }
}